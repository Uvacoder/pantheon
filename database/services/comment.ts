...
const CommentRepo = (orm: MikroORM) => orm.em.getRepository(Comment);
...
const CommentService = {
    ...
    async findById(id: string) {
        ...
        return await CommentRepo(orm).findOne({ id: id }, ["commenter"]);
    },

    async findManyByIds(ids: string[]) {
        const orm = await getOrm();

        return await CommentRepo(orm).findOne({ id: { $in: ids } });
    },

    async findTreesByFilter(filter: CommentTreeFilter) {
        const orm = await getOrm();

        const roots = await CommentRepo(orm).find({
            post: filter.post,
            path: ""
        }, ["commenter"],
            filter.sort === "top" ?
    ...
    async findTreeByPath(path: string[]) {
        ...
        return await CommentRepo(orm).find({ path: { $like: compare } }, ["commenter"]);
    },

    async findByFilter(filter: CommentFilter) {
        const orm = await getOrm();

        const comments = await CommentRepo(orm).find(
    ...
    async findByIdAndCommenter(commentQuery: CommentQuery) {
        const orm = await getOrm();
        return await CommentRepo(orm).findOne({ id: commentQuery.id, commenter: commentQuery.commenter });
    },
